...
def save_figure(fig, name):
    ...
    mng.full_screen_toggle()
    print(f"Saved to {RESDIR}{name}.png")
...
def select_points(img, K):
    ...
    plt.close(fig)
    return [tuple(map(int, tup)) for tup in x]
...
def select_pair(img1, img2, K, win1, win2, pts1, pts2, print_help=True):
    if print_help:
        print(
            "Select the corresponding %d points" % (K),
...
def select_corresponding_points(dataset, K):
    ...
    result = [[None for _ in range(len(imgs))] for _ in range(len(imgs))]
    print("Select the corresponding %d points" % (K), "in each image in the same order")
    ...
    for i in range(len(imgs) - 1):
        for j in range(i + 1, len(imgs)):
            ...
            result[i][j] = select_pair(
                img1, img2, K, win1, win2, [], [], print_help=False
            )
...
def random_points(size, K):
    ...
    return [tuple(map(int, arr)) for arr in x]
...
def mean_blend(img1, img2):
    ...
    return cv.addWeighted(blended1, 0.5, blended2, 0.5, 0)
...
def color_list(N, colormap=cv.COLORMAP_HSV):
    ...
    return [tuple(int(c) for c in cmap[int(256 * n / N)][0]) for n in range(N)]
...
def mark_points(img, x, colormap=cv.COLORMAP_HSV, bgr=False):
    ...
    return marked if bgr else cv.cvtColor(marked, cv.COLOR_BGR2RGB)
...
def show_matches(dataset, points):
    ...
    for i in range(len(imgs) - 1):
        for j in range(i + 1, len(imgs)):
            ...
            plt.figure()
            plt.imshow(img)
            plt.title(f"{IMNAMES[dataset][i]} - {IMNAMES[dataset][j]}")
...
def test_save_points(dataset):
    print(f"Running test_save_points for {dataset} dataset")
...
def test_check_points(dataset, h_use_cv=False, w_use_cv=False):
    print(f"Running test_check_points for {dataset} dataset")
        ...
        while j + 1 < len(points):
            ...
            if len(pts1):
                ...
                if ch == 13:
                    ...
                    points[i][j] = select_pair(
                        img1, img2, len(pts1), win1, win2, list(pts1), list(pts2)
                    )
                    save_points(dataset, points)
                    points = load_points(dataset)
                    j -= 1
...
def test_image_pair(dataset, id_from, id_to, den=1, h_use_cv=False, w_use_cv=False):
    print(f"Running test_image_pair for {dataset} dataset")
    print(
        "Stitching from #%d to #%d with a downscale factor of %.1f"
        % (id_from, id_to, den)
    )
    print(f"OpenCV usage: homography ({h_use_cv}), warping ({w_use_cv})")
    imgs = read_imgs(dataset)
...
def test_stitch_all(dataset, den=4):
    print(f"Running test_stitch_all for {dataset} dataset")
    ...
    plt.imshow(pano)
    plt.title(f"All {dataset} Images Stitched")
    plt.show()
...
def task_mosaic_5(dataset, method, den=1):
    print(f"Running task_mosaic_5 for {dataset} dataset")
    ...
    if method is METHODS[0]:  # left-to-right
        pano = _extracted_from_task_mosaic_5_18(points, imgs)
    elif method is METHODS[1]:  # middle-out
        pano = _extracted_from_task_mosaic_5_42(points, imgs)
    else:  # first-out-then-middle
        pano = _extracted_from_task_mosaic_5_69(points, imgs)
    # display and save result
    plt.imshow(pano)
    plt.title(f"{dataset} Mosaic with {method} Method")
    plt.show()
    save_image(f"mosaic_{dataset}_{method}", pano)


# TODO Rename this here and in `task_mosaic_5`
def _extracted_from_task_mosaic_5_69(points, imgs):
    # left2 to left1
    pts1 = points[0][1][0]
    pts2 = points[0][1][1]
    H = computeH(pts1, pts2)
    ...
    # all together
    result, pos = patchPano(pano_left, pano_right, pos_left, pos_right)
    return result


# TODO Rename this here and in `task_mosaic_5`
def _extracted_from_task_mosaic_5_42(points, imgs):
    # left1 to middle
    pts1 = points[1][2][0]
    pts2 = points[1][2][1]
    H = computeH(pts1, pts2)
    ...
    # mosaic2
    result, pos = patchPano(pano0, pano4, pos0, pos4)
    # all together
    result, pos = patchPano(result, pano2, pos, pos2)
    return result


# TODO Rename this here and in `task_mosaic_5`
def _extracted_from_task_mosaic_5_18(points, imgs):
    # left1 to left2
    pts1 = points[1][0][0]
    pts2 = points[1][0][1]
    H = computeH(pts1, pts2)
    ...
    result, pos = patchPano(pano1, pano2, pos1, pos2)
    # right1 to left2
    pts1 = points[3][0][0]
    pts2 = points[3][0][1]
    H = computeH(pts1, pts2)
    pano3, pos3 = blend_images(imgs[3], imgs[0], H)
    result, pos = patchPano(result, pano3, pos, pos3)
    # right2 to left2
    pts1 = points[4][0][0]
    pts2 = points[4][0][1]
    H = computeH(pts1, pts2)
    pano4, pos4 = blend_images(imgs[4], imgs[0], H)
    result, pos = patchPano(result, pano4, pos, pos4)
    return result
...
def task_corresponding_pairs(id_from, id_to, K=8):
    ...
    # load data
    dataset = "paris"
    print(f"Running task_corresponding_pairs for {dataset} dataset")
    print(
        "Stitching from #%d to #%d with %d corresponding points" % (id_from, id_to, K)
    )
    imgs = read_imgs(dataset)
    points = load_points(dataset)
...
def task_point_selection(id_from, id_to, wrong=5):
    ...
    # load data
    dataset = "paris"
    print(f"Running task_point_selection for {dataset} dataset")
    print(
        "Stitching from #%d to #%d with %d wrong corresponding points"
        % (id_from, id_to, wrong)
    )
    imgs = read_imgs(dataset)
    points = load_points(dataset)
...
def task_noisy_points(id_from, id_to, noise=5):
    # load data
    dataset = "paris"
    print(f"Running task_noisy_points for {dataset} dataset")
    print(
        "Stitching from #%d to #%d with a noise of %d pixels" % (id_from, id_to, noise)
    )
    imgs = read_imgs(dataset)
    points = load_points(dataset)
    img1 = imgs[id_from]
...
def task_normalized_points(id_from, id_to):
    # load data
    dataset = "paris"
    print(f"Running task_normalized_points for {dataset} dataset")
